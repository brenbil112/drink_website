const shotRecipes = {
    "Alabama Slammer": {
        ingredients: ["Southern Comfort", "Amaretto", "Sloe Gin", "Orange Juice"],
    },
    "Apple Pie Shot (Fireball)": {
        ingredients: ["Fireball", "Apple Cider"],
    },
    "Apple Pie Shot (Bourbon)": {
        ingredients: ["Bourbon", "Apple Cider", "Cinnamon"],
    },
    "B-52": {
        ingredients: ["Kahlúa", "Baileys", "Grand Marnier"],
    },
    "Baby Guinness": {
        ingredients: ["Kahlúa", "Baileys"],
    },
    "Banana Bread Shot": {
        ingredients: ["Banana Liqueur", "Frangelico", "Whipped Cream"],
    },
    "Bazooka Joe": {
        ingredients: ["Vodka", "Blue Curacao", "Sour Mix"],
    },
    "Big Bang": {
        ingredients: ["Vodka", "Blue Curacao", "Red Bull"],
    },
    "Black Russian": {
        ingredients: ["Vodka", "Kahlúa"],
    },
    "Blow Job": {
        ingredients: ["Baileys", "Amaretto", "Whipped Cream"],
    },
    "Boilermaker"   : {
        ingredients: ["Rye", "Beer"],
    },
    "Bourbon Cherry": {
        ingredients: ["Bourbon", "Cherry Liqueur"],
    },
    "Brain Freeze": {
        ingredients: ["Vodka", "Blue Curacao", "Sour Mix"],
    },
    "Breakfast Shot": {
        ingredients: ["Jameson", "Butterscotch Schnapps", "Orange Juice"],
    },
    "Buttery Nipple": {
        ingredients: ["Butterscotch Schnapps", "Baileys"],
    },
    "Cement Mixer": {
        ingredients: ["Baileys", "Lime Juice"],
    },
    "Chocolate Cake": {
        ingredients: ["Frangelico", "Vodka", "Sugar Rim"],
    },
    "Cinnamon Toast Crunch": {
        ingredients: ["Fireball", "Rumchata"],
    },
    "Cobra's Fang": {
        ingredients: ["Rum", "Orange Juice", "Grenadine"],
    },
    "Duck Fart": {
        ingredients: ["Rye", "Baileys", "Kahlúa"],
    },
    "Gimlet Shot": {
        ingredients: ["Gin", "Lime Juice"],
    },
    "Goldschläger Shot": {
        ingredients: ["Goldschläger", "Rumple Minze"],
    },
    "Green Tea Shot": {
        ingredients: ["Jameson", "Peach Schnapps", "Sour Mix"],
    },
    "Gummy Bear": {
        ingredients: ["Vodka", "Peach Schnapps", "Sour Mix", "Sprite"],
    },
    "Irish Car Bomb": {
        ingredients: ["Irish Whiskey", "Guinness", "Irish Cream"],
    },
    "Jägerbomb": {
        ingredients: ["Jägermeister", "Red Bull"],
    },
    "Jolly Rancher": {
        ingredients: ["Vodka", "Apple Pucker", "Cranberry Juice"],
    },
    "Kamikaze": {
        ingredients: ["Vodka", "Triple Sec", "Lime Juice"],
    },
    "Kool Aid": {
        ingredients: ["Vodka", "Amaretto", "Cranberry Juice"],
    },
    "Lemon Drop": {
        ingredients: ["Vodka", "Lemon Juice", "Simple Syrup"],
    },
    "Mexican Candy": {
        ingredients: ["Tequila", "Watermelon Liqueur", "Hot Sauce"],
    },
    "Mind Eraser": {
        ingredients: ["Vodka", "Kahlúa", "Club Soda"],
    },
    "Mini Beer": {
        ingredients: ["Licor 43", "Baileys"],
    },
    "New York Sour": {
        ingredients: ["Bourbon", "Lemon Juice", "Red Wine"],
    },
    "Oatmeal Cookie Shot": {
        ingredients: ["Baileys", "Goldschläger", "Butterscotch Schnapps"],
    },
    "Peanut Butter & Jelly": {
        ingredients: ["Vodka", "Frangelico", "Cranberry Juice"],
    },
    "Peppermint Patty": {
        ingredients: ["Peppermint Schnapps", "Baileys"],
    },
    "Pickleback": {
        ingredients: ["Rye", "Pickle Juice"],
    },
    "Pineapple Upside Down Cake": {
        ingredients: ["Rum", "Pineapple Juice", "Grenadine"],
    },
    "Pink Starburst": {
        ingredients: ["Vodka", "Strawberry Liqueur", "Sour Mix"],
    },
    "Polar Bear": {
        ingredients: ["Vodka", "Blue Curacao", "Cream"],
    },
    "Rum Ball Shot": {
        ingredients: ["Rum", "Chocolate Liqueur", "Cream"],
    },
    "Russian Spring Punch": {
        ingredients: ["Vodka", "Lemon Juice", "Simple Syrup", "Sparkling Wine"],
    },
    "Sangrita": {
        ingredients: ["Tequila", "Tomato Juice", "Lime Juice", "Hot Sauce"],
    },
    "Scooby Snack": {
        ingredients: ["Rum", "Midori", "Pineapple Juice", "Cream"],
    },
    "Sour Patch Kid": {
        ingredients: ["Vodka", "Sour Mix", "Grenadine"],
    },
    "Tequila Slammer": {
        ingredients: ["Tequila", "Sprite", "Grenadine"],
    },
    "Tequila Sunrise Shot": {
        ingredients: ["Tequila", "Orange Juice", "Grenadine"],
    },
    "Tree Trimmer": {
        ingredients: ["Bourbon", "Peppermint Schnapps", "Crème de Menthe"],
    },
    "Water Moccasin": {
        ingredients: ["Rye", "Peach Schnapps", "Sour Mix"],
    },
    "White Tea Shot": {
        ingredients: ["Vodka", "Peach Schnapps", "Sour Mix", "Sprite"],
    },
 
}